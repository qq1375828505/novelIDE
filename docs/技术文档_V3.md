...
Android版是完整移动端小说写作IDE，底部导航以作品创作为中心，不以AI聊天为中心。
...
NavItem(icon: Icons.person_outline, label: '我的', page: ProfilePage()),
```
...
AI不固定为主导航Tab，而是作为写作动作出现：
- 正文选中后：润色、扩写、改对白、续写、检查水文
- 当前章节内：总结本章、生成标题、生成爽点、生成下一章钩子
- AI抽屉：自由聊天、脑暴剧情、插入生成内容到当前章节
  ...
  onPressed: () async {
    if (isListening) {
      ...
      await speechToText.listen(
        onResult: (result) => insertText(result.recognizedWords),
        localeId: 'zh_CN',
      );
    }
  },
...
│   │   ├── materials/materials_page.dart   # 资料管理（7个Tab）
│   │   ├── outline/outline_page.dart       # 大纲（含主线大纲编辑）
│   │   ├── profile/profile_page.dart       # 我的（AI配置+统计+对话入口）
...
│   │   │   └── title_generator_result_page.dart # ⭐ 标题生成结果
│   │   ├── works/
│   │   │   ├── works_page.dart             # 作品列表
│   │   │   ├── novel_detail_page.dart      # 卷章管理
│   │   │   └── export_page.dart            # ⭐ 勾选式导出系统
│   │   └── writing/
│   │       ├── writing_page.dart
│   │       └── editor_page.dart            # 编辑器（自动保存+记忆更新+undo/redo）
│   └── state/
...
AI 对话作为独立 Tab（写作/作品/大纲/资料/我的/**AI对话**），不再是子页面。
...
- ✅ AppBar 右上角 + 底部工具栏手动保存按钮
- ✅ 底部工具栏：undo/redo + 保存（移除语音按钮）
