...
- **文件附件** — 支持通过文件选择器上传图片
- **选择资料上下文** — 从资料库选择角色/设定/章节/伏笔，自动拼入对话上下文发给AI
- **去AI味** — 一键将输入框文本送去AI按Humanizer规则改写
...
- **去AI味 Agent** — 基于维基百科AI写作特征指南，自动检测并消除AI写作痕迹（过度强调词/空洞评价/三项排比/破折号滥用/虚假归因等8大规则）
- **Agent工具** — 35+工具执行器，覆盖读取/写入/编辑/删除/分析全操作
...
#### 5.1 人物关系图
- **可视化关系网络** — 角色节点 + 关系连线，Canvas绘制
- **可拖动节点** — 自由调整角色位置
- **角色颜色** — 按身份着色（主角蓝/女主粉/反派红/师父绿/配角灰）
- **18种预设关系** — 父女/师徒/敌人/恋人/同门/主仆等
- **位置持久化** — 关系图布局保存到JSON文件
- **入口** — 资料树"角色"文件夹长按 → "查看关系图"

#### 5.2 自定义文件夹
- **创建分类** — 用户可创建自定义资料文件夹
- **持久化存储** — 自定义文件夹保存为JSON，重启不丢失
- **长按操作** — 重命名/删除/添加条目
- **条目编辑** — 长按资料条目 → 编辑/删除
